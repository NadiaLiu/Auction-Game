...
class AuctionClient(object):
    ...
    def first_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        budget = standings[mybidderid]['money']

        # Find the target artist
        owned_items = {}
        for artist in artists.keys():
            if standings[mybidderid][artist] > 0:
                owned_items[artist] = standings[mybidderid][artist]

        if len(owned_items) != 0:
            target_item = max(owned_items, key=owned_items.get)
            target_number = standings[mybidderid][target_item]
        else:
            target_item = None

        # If I have already had a item, keep bid this artist,
        # and always bid average of budget when current item is our target
        if target_item is not None:
            if current_item == target_item:
                return int(budget / (wincondition - target_number))

        # Take risks when others is going to win if I have less than 3 items

        # find the item and money of potential opponent
        risk_artist = {}
        for player in players:
            if player != mybidderid:
                for artist in artists.keys():
                    if standings[player][artist] == 4:
                        risk_artist[artist] = standings[player]['money']

        if current_item in risk_artist.keys():
            if target_item is not None and target_number <= 3 and budget >= risk_artist[current_item]:
                return risk_artist[current_item] + 1
            else:
                return 0

        # If I have not had a item, bid the artist which first occur 6 times
        artist_count = {}
        for artist in artists.keys():
            artist_count[artist] = 0

        for round in itemsinauction[rd:]:
            artist_count[round] += 1
            for artist in artists.keys():
                if artist_count[artist] == 6:
                    target_item = artist

        if current_item == target_item:
            return int(budget / wincondition)
        else:
            return 0
    ...
    def second_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        # If I have not had a item, bid the artist which is most in left auction
        # Calculate left items
        left_items = artists.copy()
        for i in itemsinauction[:rd]:
            left_items[i] -= 1
        target_item = max(left_items, key=left_items.get)
    ...
    def third_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        # Only bid Rembrandt and Da_Vinci, calculate the total value
        target_item = ['Rembrandt','Da_Vinci']
        artist_left = {}
        for artist in target_item:
            artist_left[artist] = 0
        ...
        total_value = 0
        for artist in target_item:
            total_value += values[artist] * artist_left[artist]

        # Calculate the utility of current item
        if current_item in target_item:
            v = int(budget * (values[current_item] / total_value))
            return v
        else:
